...
public class SkulHeadController : MonoBehaviour
{
	...
	private void Awake()
	{
		_skulSpritesMap = _skulSprites.ToDictionary((Sprite s) => ((Object)s).name);
		_skulHeadlessSpritesMap = _skulHeadlessSprites.ToDictionary((Sprite s) => ((Object)s).name);
	}
	...
	private void LateUpdate()
	{
		...
		{
			if (_skulSpritesMap.TryGetValue(((Object)_spriteRenderer.sprite).name, out value))
			{
				_spriteRenderer.sprite = value;
			}
		}
		else if (_skulHeadlessSpritesMap.TryGetValue(((Object)_spriteRenderer.sprite).name, out value))
		{
			_spriteRenderer.sprite = value;
		}
	}
