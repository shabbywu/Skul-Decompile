...
public class CombatAction : MonoBehaviour
{
	private class MightyBlow
	{
		...
		internal void UpdateStat()
		{
			...
			if (_streaks > 5)
			{
				_movementSpeed.value = 1.100000023841858;
				duration = 3f;
			}
			else if (_streaks > 7)
			{
	...
	private class Massacre
	{
		...
		internal void UpdateStat()
		{
			...
			{
				_movementSpeed.value = 1.399999976158142;
				duration = 10f;
			}
			else if (streaks > 50)
			{
	...
	{
		if (!((Object)(object)target.character == (Object)null) && !(target.character.health.currentHealth > 0.0))
		{
			_mightyBlow.streaks++;
			_massacre.streaks++;
		}
