...
public class FanaticFactory : MonoBehaviour
{
	...
	public void DropFanatics()
	{
		...
		{
			((MonoBehaviour)this).StartCoroutine(ladder.CFall());
		}
	}
	...
	public IEnumerator CProcess()
	{
		...
		{
			Debug.LogError((object)"Fanatics count is 0");
			yield break;
		}
		while (_running)
		{
			...
			foreach (FanaticLadder ladder in ladders)
			{
				((MonoBehaviour)this).StartCoroutine(CSummon(ladder));
			}
	...
	private IEnumerator CSummon(FanaticLadder ladder)
	{
		...
		if (_running)
		{
			Character spawned = Object.Instantiate<Character>(_fanatics[ladder.fanatic], ladder.spawnPoint, Quaternion.identity, _config.spawnedContainer);
