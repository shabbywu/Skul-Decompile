...
public class ParentPool : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			((Component)parent.range).gameObject.SetActive(false);
		}
	}
	...
	private void PickOneAttackRange()
	{
		foreach (EffectrRangeKeyValue parent in _parents)
		{
			...
			if ((Object)(object)range != (Object)(object)_currentAttackParent)
			{
				((Component)range).gameObject.SetActive(false);
			}
			else
			{
				((Component)range).gameObject.SetActive(true);
			}
		}
	}
}
