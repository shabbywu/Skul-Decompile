...
public class RockstarMusic : MonoBehaviour
{
	...
	private class AmpMusic
	{
		...
		private IEnumerator CPlayAmpSound()
		{
			...
			{
				int timingIndex;
				yield return (object)new WaitForSeconds(GetNextEffectTime(out timingIndex) - Time.time);
				if (!pause)
				{
	...
	private void Awake()
	{
		...
		{
			array[j].Initialize((MonoBehaviour)(object)this);
		}
	}
	...
	private void PlaySolo()
	{
		...
		_soloSource = PersistentSingleton<SoundManager>.Instance.PlaySound(_solo, ((Component)this).transform.position);
	}

	private void SetSoloVolume(float volume)
	{
		if (!((Object)(object)_soloSource == (Object)null))
		{
	...
	private void StartSolo()
	{
		...
		{
			SetBeat(_soloBpm);
			_loopSound = ((MonoBehaviour)this).StartCoroutine(CLoopSoundWhileAction());
		}
	}
	...
	{
		_running = false;
		ReusableAudioSource soloSource = _soloSource;
		if (soloSource != null)
		{
			soloSource.Stop();
		}
	...
	private void StartPassive()
	{
		...
		((MonoBehaviour)this).StartCoroutine(CWaitForPassiveFinish());
	}
