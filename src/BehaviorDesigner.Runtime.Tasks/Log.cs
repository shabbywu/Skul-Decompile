...
public class Log : Action
{
	...
	public override TaskStatus OnUpdate()
	{
		...
		{
			Debug.LogError((object)(((SharedVariable<bool>)logTime).Value ? ((SharedString)$"{Time.time}: {text}") : text));
		}
		else
		{
			Debug.Log((object)(((SharedVariable<bool>)logTime).Value ? ((SharedString)$"{Time.time}: {text}") : text));
		}
		return (TaskStatus)2;
	}
