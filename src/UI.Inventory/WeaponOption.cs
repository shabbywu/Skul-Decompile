...
public class WeaponOption : MonoBehaviour
{
	...
	public void Set(Weapon weapon)
	{
		...
		_rarity.text = Localization.GetLocalizedString(string.Format("{0}/{1}/{2}", "label", "Rarity", weapon.rarity));
		...
		((Component)_category).gameObject.SetActive(true);
	...
	private void Update()
	{
		...
		else if (!_detailContainer.activeSelf && ((OneAxisInputControl)KeyMapper.Map.UiInteraction3).IsPressed && !((OneAxisInputControl)KeyMapper.Map.UiInteraction2).IsPressed)
		{
