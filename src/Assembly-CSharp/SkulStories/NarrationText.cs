...
public class NarrationText : MonoBehaviour
{
	...
	public IEnumerator CFadeInText(string text)
	{
		...
		_intactText = ((TMP_Text)_text).text;
		for (int index = 0; index < 256; index += _textSpeed)
		{
			...
			((TMP_Text)_text).text = _display.ToString();
			yield return null;
			_alpha.Clear();
			_display.Clear();
			if (_skippable)
	...
	public void InsertText()
	{
		...
		{
			((TMP_Text)_text).text = ((TMP_Text)_text).text.Insert(((TMP_Text)_text).text.Length, narration);
		}
	}
	...
	{
		_skippable = true;
		((TMP_Text)_text).text = _intactText;
	}
