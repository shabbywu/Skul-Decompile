...
public class GetBone : Sequence
{
	...
	{
		_inventory = ((Component)_player).GetComponent<WeaponInventory>();
		_tutorialSkul = ((Component)_inventory.polymorphOrCurrent).GetComponent<TutorialSkul>();
		_tutorialSkul.getBone.TryStart();
		yield return Chronometer.global.WaitForSeconds(0.5f);
		((MonoBehaviour)this).StartCoroutine(CGetWeapon());
	}
	...
	private IEnumerator CGetWeapon()
	{
		...
		((Component)Singleton<Service>.Instance.levelManager.player).GetComponent<WeaponInventory>().ForceEquipAt(_skul.Instantiate(), 0);
