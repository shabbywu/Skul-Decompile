...
public class Mark : MonoBehaviour
{
	...
	{
		Mark mark = ((Component)owner).gameObject.AddComponent<Mark>();
		mark.owner = owner;
	...
	public void AddStack(MarkInfo mark, float count = 1f)
	{
		...
		if (num == -1)
		{
			...
			EffectPoolInstance item = stackImageBase.Spawn(((Component)this).transform.position, owner);
	...
	public void ClearAllStack()
	{
		_markInfo.Clear();
		_stacks.Clear();
		foreach (EffectPoolInstance stackImage in _stackImages)
		{
			stackImage.Stop();
		}
		_stackImages.Clear();
