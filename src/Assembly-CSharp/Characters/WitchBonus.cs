...
public class WitchBonus
{
	public abstract class Bonus
	{
		...
		public string displayName => Localization.GetLocalizedString(_key);
		...
		{
			return Localization.GetLocalizedString(_key + "/desc");
		}
	}
	...
	public class StatBonus : Bonus
	{
		...
		{
			return string.Format(Localization.GetLocalizedString(_key + "/desc"), _statPerLevel.value * (double)level);
		}
	...
	public class GenericBonus : Bonus
	{
		...
		public override string GetDescription(int level)
		{
			return string.Format(Localization.GetLocalizedString(_key + "/desc"), _bonusPerLevel * (float)level);
		}
	...
	public class GetShieldOnSwap : GenericBonus
	{
		...
		public override string GetDescription(int level)
		{
			return string.Format(Localization.GetLocalizedString(_key + "/desc"), _bonusPerLevel * (float)level, _duration, _swapCooldownReduction);
		}
	}
	...
	public class ReviveOnce : Bonus, IAbility, IAbilityInstance
	{
		...
		private static readonly TargetLayer _layer = new TargetLayer(LayerMask.op_Implicit(0), allyBody: false, foeBody: true, allyProjectile: false, foeProjectile: false);
			...
			: base(key, owner, data, costs)
		{
			...
			icon = CommonResource.instance.reassembleIcon;
		}
		...
		private void Revive()
		{
			...
			CircleCaster val = new CircleCaster
			{
				origin = Vector2.op_Implicit(((Component)_owner).transform.position),
				radius = 8f
			};
			...
			for (int i = 0; i < components.Count; i++)
			{
				...
				{
					Damage damage = new Damage(_owner, 50.0, MMMaths.RandomPointWithinBounds(((Collider2D)character.collider).bounds), Damage.Attribute.Fixed, Damage.AttackType.Additional, Damage.MotionType.Item, base.level, 0.5f, 0.0, 1.0, 1.0, canCritical: true, @null: false, 0.0, 0.0, 0);
					character.movement.push.ApplyKnockback(_owner, new Vector2(0f, 5f), new Curve(AnimationCurve.Linear(0f, 0f, 1f, 1f), 0.3f, 0.5f));
					_owner.TryAttackCharacter(new TargetStruct(character), ref damage);
				}
			}
		}
		...
		public override string GetDescription(int level)
		{
			return string.Format(Localization.GetLocalizedString(_key + "/desc"), _remainhealthPercent * (float)level);
		}
	}
