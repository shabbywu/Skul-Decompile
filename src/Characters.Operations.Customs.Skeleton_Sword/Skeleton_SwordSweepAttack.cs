			...
			for (int i = 0; i < _caster.results.Count; i++)
			{
				...
				void HandleResult()
				{
					...
					else
					{
						...
						{
							Debug.LogError((object)(((Object)((RaycastHit2D)(ref result)).collider).name + " : Character has no Target component"));
						}
						else if (_hits.CanAttack(target, _maxHits, _maxHitsPerUnit, _hitIntervalPerUnit))
						{
	...
	protected virtual IEnumerator CAttack(Vector2 origin, Vector2 direction, float distance, RaycastHit2D raycastHit, Target target)
	{
		...
		while ((Object)(object)this != (Object)null && index < attackAndEfects.Length)
		{
			for (; index < attackAndEfects.Length; index++)
			{
				...
				if (!(time >= (castAttackInfoSequence2 = attackAndEfects[index]).timeToTrigger))
				{
					break;
				}
